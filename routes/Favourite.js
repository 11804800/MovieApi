...
.post(authenticate.verifyUser,(req,res,next)=>{
    if(req.body!=null)
    {
        ...
        .then((comment)=>{
            Favourite.findById(comment._id)
            .populate('author')
            .then((comment)=>{
                res.statuscode=200;
...
.delete(authenticate.verifyUser,async (req, res) => {
    try{
        const resp=await Favourite.remove({});
...
FavouriteRouter.route('/:id')
.get(async (req, res) => {
    try{
        const movie=await Favourite.findOne(req.params.id)
        .populate('User');
...
.delete(authenticate.verifyUser, async (req, res) => {
    try{
        const resp=await Favourite.findOneAndRemove(req.params.id);
        res.status(200).json(resp);
    }
    catch (err) {
        res.status(500).send(err);
